...
def main(base_dir, no_samples,sample_type):
    ...
    sample(join(base_dir,"male"),no_samples,join("data",sample_type,"male"))

    sample(join(base_dir,"female"),no_samples,join("data",sample_type,"female"))
...
def sample(in_dir, no_samples, out_dir):
    ...
    for x in range(no_samples):
        ...
        while empty:
            ...
            ang_choice = join(choice(subjects), choice(variations), choice(angles),"aligned")
        ...
        copyfile(sample, join(out_dir,"%03d.png" % (x + 1)))
